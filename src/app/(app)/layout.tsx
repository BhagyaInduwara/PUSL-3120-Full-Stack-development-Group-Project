...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <SocketProvider>
      <div className="flex h-screen bg-[var(--color-bg)] text-[var(--color-text)] font-[family-name:var(--font-body)] overflow-hidden transition-colors duration-200">
        <Sidebar user={user.toPublic()} />
        <div className="flex-1 flex flex-col overflow-hidden min-w-0 bg-ambient-glow">
          <OfflineBanner />
          {children}
        </div>
      </div>
